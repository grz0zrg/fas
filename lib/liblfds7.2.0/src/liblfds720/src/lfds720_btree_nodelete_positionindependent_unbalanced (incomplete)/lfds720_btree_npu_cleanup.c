                                ...
                                void (*element_cleanup_callback)(struct lfds720_btree_npu_state *baus, struct lfds720_btree_npu_element *baue) )
{
  enum lfds720_btree_npu_delete_action
    delete_action = LFDS720_BTREE_NPU_DELETE_SELF; // TRD : to remove compiler warning
  ...
  */
  ...
  lfds720_btree_npu_get_by_absolute_position( baus, &baue, LFDS720_BTREE_NPU_ABSOLUTE_POSITION_ROOT );

  while( baue != NULL )
  {
    if( baue->left == NULL and baue->right == NULL )
      delete_action = LFDS720_BTREE_NPU_DELETE_SELF;

    if( baue->left != NULL and baue->right == NULL )
      delete_action = LFDS720_BTREE_NPU_DELETE_SELF_REPLACE_WITH_LEFT_CHILD;

    if( baue->left == NULL and baue->right != NULL )
      delete_action = LFDS720_BTREE_NPU_DELETE_SELF_REPLACE_WITH_RIGHT_CHILD;

    if( baue->left != NULL and baue->right != NULL )
      delete_action = LFDS720_BTREE_NPU_DELETE_MOVE_LEFT;

    switch( delete_action )
    {
      case LFDS720_BTREE_NPU_DELETE_SELF:
        ...
        temp = baue;
        lfds720_btree_npu_get_by_relative_position( &baue, LFDS720_BTREE_NPU_RELATIVE_POSITION_UP );
        element_cleanup_callback( baus, temp );
      break;

      case LFDS720_BTREE_NPU_DELETE_SELF_REPLACE_WITH_LEFT_CHILD:
        ...
        temp = baue;
        lfds720_btree_npu_get_by_relative_position( &baue, LFDS720_BTREE_NPU_RELATIVE_POSITION_LEFT );
        element_cleanup_callback( baus, temp );
      break;

      case LFDS720_BTREE_NPU_DELETE_SELF_REPLACE_WITH_RIGHT_CHILD:
        ...
        temp = baue;
        lfds720_btree_npu_get_by_relative_position( &baue, LFDS720_BTREE_NPU_RELATIVE_POSITION_RIGHT );
        element_cleanup_callback( baus, temp );
      break;

      case LFDS720_BTREE_NPU_DELETE_MOVE_LEFT:
        lfds720_btree_npu_get_by_relative_position( &baue, LFDS720_BTREE_NPU_RELATIVE_POSITION_LEFT );
      break;
    }
  }
