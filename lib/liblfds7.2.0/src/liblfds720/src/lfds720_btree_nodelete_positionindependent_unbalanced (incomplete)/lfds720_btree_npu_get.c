...
/***** private prototypes *****/
static void lfds720_btree_npu_internal_inorder_walk_from_largest_get_next_smallest_element( struct lfds720_btree_npu_element **baue );
static void lfds720_btree_npu_internal_inorder_walk_from_smallest_get_next_largest_element( struct lfds720_btree_npu_element **baue );
                                                ...
                                                enum lfds720_btree_npu_absolute_position absolute_position )
{
  ...
  *baue = baus->root;
  ...
  {
    case LFDS720_BTREE_NPU_ABSOLUTE_POSITION_ROOT:
    break;

    case LFDS720_BTREE_NPU_ABSOLUTE_POSITION_LARGEST_IN_TREE:
      if( *baue != NULL )
    ...
    case LFDS720_BTREE_NPU_ABSOLUTE_POSITION_SMALLEST_IN_TREE:
      if( *baue != NULL )
                                                ...
                                                enum lfds720_btree_npu_relative_position relative_position )
{
  ...
  {
    case LFDS720_BTREE_NPU_RELATIVE_POSITION_UP:
      *baue = (*baue)->up;
      // TRD : no load barrier - up already existed, so is known to be safely propagated
    break;

    case LFDS720_BTREE_NPU_RELATIVE_POSITION_LEFT:
      *baue = (*baue)->left;
      LFDS720_MISC_BARRIER_LOAD;
    break;

    case LFDS720_BTREE_NPU_RELATIVE_POSITION_RIGHT:
      *baue = (*baue)->right;
      LFDS720_MISC_BARRIER_LOAD;
    break;

    case LFDS720_BTREE_NPU_RELATIVE_POSITION_SMALLEST_ELEMENT_BELOW_CURRENT_ELEMENT:
      *baue = (*baue)->left;
      if( *baue != NULL )
      {
        LFDS720_MISC_BARRIER_LOAD;
    ...
    case LFDS720_BTREE_NPU_RELATIVE_POSITION_LARGEST_ELEMENT_BELOW_CURRENT_ELEMENT:
      *baue = (*baue)->right;
      if( *baue != NULL )
      {
        LFDS720_MISC_BARRIER_LOAD;
    ...
    case LFDS720_BTREE_NPU_RELATIVE_POSITION_NEXT_SMALLER_ELEMENT_IN_ENTIRE_TREE:
      lfds720_btree_npu_internal_inorder_walk_from_largest_get_next_smallest_element( baue );
    break;

    case LFDS720_BTREE_NPU_RELATIVE_POSITION_NEXT_LARGER_ELEMENT_IN_ENTIRE_TREE:
      lfds720_btree_npu_internal_inorder_walk_from_smallest_get_next_largest_element( baue );
    break;
  }
...
static void lfds720_btree_npu_internal_inorder_walk_from_largest_get_next_smallest_element( struct lfds720_btree_npu_element **baue )
{
  enum lfds720_btree_npu_move
    action = LFDS720_BTREE_NPU_MOVE_INVALID;
  ...
  */
  ...
  if( left != NULL )
    action = LFDS720_BTREE_NPU_MOVE_LARGEST_FROM_LEFT_CHILD;

  if( left == NULL and up != NULL and up_right == *baue )
    action = LFDS720_BTREE_NPU_MOVE_GET_PARENT;

  if( (left == NULL and up == NULL) or (up != NULL and up_left == *baue and left == NULL) )
    action = LFDS720_BTREE_NPU_MOVE_MOVE_UP_TREE;

  switch( action )
  {
    case LFDS720_BTREE_NPU_MOVE_INVALID:
    case LFDS720_BTREE_NPU_MOVE_SMALLEST_FROM_RIGHT_CHILD:
      // TRD : eliminates a compiler warning
    break;

    case LFDS720_BTREE_NPU_MOVE_LARGEST_FROM_LEFT_CHILD:
      *baue = left;
      if( *baue != NULL )
      {
        LFDS720_MISC_BARRIER_LOAD;
    ...
    case LFDS720_BTREE_NPU_MOVE_GET_PARENT:
      *baue = up;
    break;

    case LFDS720_BTREE_NPU_MOVE_MOVE_UP_TREE:
...
static void lfds720_btree_npu_internal_inorder_walk_from_smallest_get_next_largest_element( struct lfds720_btree_npu_element **baue )
{
  ...
  */
  ...
  if( right != NULL )
    action = LFDS720_BTREE_NPU_MOVE_SMALLEST_FROM_RIGHT_CHILD;
                                                                              ...
                                                                              enum lfds720_btree_npu_relative_position relative_position )
{
  ...
  if( *baue == NULL )
    rv = lfds720_btree_npu_get_by_absolute_position( baus, baue, absolute_position );
  else
    rv = lfds720_btree_npu_get_by_relative_position( baue, relative_position );
