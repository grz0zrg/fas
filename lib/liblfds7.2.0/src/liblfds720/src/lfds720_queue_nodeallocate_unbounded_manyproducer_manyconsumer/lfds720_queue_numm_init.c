                                                           ...
                                                           void *user_state )
{
  ...
  */
  ...
  qumms->enqueue[LFDS720_MISC_POINTER] = qumme_dummy;
  qumms->enqueue[LFDS720_MISC_COUNTER] = (struct lfds720_queue_numm_element *) 0;
  qumms->dequeue[LFDS720_MISC_POINTER] = qumme_dummy;
  qumms->dequeue[LFDS720_MISC_COUNTER] = (struct lfds720_queue_numm_element *) 0;

  qumme_dummy->next[LFDS720_MISC_POINTER] = NULL;
  // TRD : no need here for an atomic add as we have a store barrier and force store below
  qumme_dummy->next[LFDS720_MISC_COUNTER] = (struct lfds720_queue_numm_element *) qumms->aba_counter++;
